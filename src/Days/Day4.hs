...
data Event = WakesUp | FallsAsleep | NewGuard Integer
    deriving (Show, Eq, Ord)
data Entry = Entry { date :: UTCTime, event :: Event }
    deriving (Show, Eq, Ord)
...
parseRecord line = let l = splitOn " " line
                       idx = (S.!!) :: [a] -> Integer -> Maybe a
                       date' = concat <$> sequence [l `idx` 0, pure " ", l `idx` 1] >>=
                            parseTimeM True defaultTimeLocale "[%Y-%m-%d %H:%M]"
                       event' = case l `idx` 2 of
...
day4 (Str input) = let entries = S.sort <$> traverse parseRecord (lines input)
                       accum ((gid, d), m) e = case event e of
                                                    ...
                                                    WakesUp      -> ((gid, date e),
                                                                ...
                                                                m)
                       entriesMap = snd . foldl' accum ((0, UTCTime (ModifiedJulianDay 0) 0), Map.empty) <$> entries
                       ...
                       sleepMaxMinute = getMaxMinute <$> (id1 >>= (\i -> entriesMap >>= Map.lookup i))
                       unfold = concatMap (\(a, b) -> [a..a + b - 1])
                       strategy1Res = (*) <$> id1 <*> (fst <$> sleepMaxMinute)
                       strategy2Res = (\(a, b) -> a * fst b) . findMaxOn (snd . snd) . 
                                        Map.toList . Map.mapMaybe (pure . getMaxMinute) <$> entriesMap
