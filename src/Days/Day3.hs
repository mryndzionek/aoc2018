module Days.Day3 (day3, day3Draw) where
...
getClaims :: Str -> [Claim]
getClaims (Str input) = let clearout = map (\c -> if c `elem` "@:x" then ',' else c) . 
                                filter (`notElem` " #")
                            parse s = read $ "(" ++ s ++ ")"
                        in parse . clearout <$> lines input

day3 :: Str -> (Int, Int)
day3 input = let claims = getClaims input
                 ...
                 (noOverlap, _, _, _, _) = let candidates = filter checkOverlap claims in
                        byPred assert "Only one candidate available" ((1 ==) . length) candidates
                            (head candidates)
              in (overlappingArea, noOverlap)

day3Draw :: Str -> IO ()
day3Draw input = let claims = getClaims input
                     (_, no) = day3 input
                     toSquare :: Claim -> (P2 Double, QDiagram B V2 Double Any)
                     toSquare (i, x, y, w, h) = let c = if i == no then red else colors !! fromIntegral (rem i 8)
